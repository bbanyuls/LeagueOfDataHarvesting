...
riot_api_fetching <- function(x) {
  key <- "RGAPI-aabdfe0a-76e1-494d-a2ff-3255122c4b2e"
  url <- paste0(x, key)
  json <- GET(url = url)
...
```{r}
accounts_per_divison <- 1 #change this to have a smaller sample
...
head(full_accounts2 |> select(tier, rank, summonerName, leaguePoints, wins, losses))
#only some of the total variables were shown for aesthetic purposes
```
...
head(matchestotal_df |> select(euwmatch, duration, teamPosition, summonerName, teamId, win), n =10) 
#only some of the total variables were shown for aesthetic purposes
...
ggplot(mapacalor2, aes(mapacalor2$position$x, y = mapacalor2$position$y))+
  ...
  scale_fill_gradient(low = "transparent", high = "red")  + # Adding fill label
  theme_void() +
 # Adjusting the color scale to go from transparent to red
...
![](https://github.com/banyulsbuc3m/LeagueOfDataHarvesting/blob/main/redmap.gif)

![](https://github.com/banyulsbuc3m/LeagueOfDataHarvesting/blob/main/blankmap.gif)
...
opgg_recent_matches("Zenki", "1310", "RGAPI-aabdfe0a-76e1-494d-a2ff-3255122c4b2e")
```
...
opgg_kda("Shakadevirgo07", "EUW", "RGAPI-aabdfe0a-76e1-494d-a2ff-3255122c4b2e")
```
...
opgg_role("Shakadevirgo07", "EUW", "RGAPI-aabdfe0a-76e1-494d-a2ff-3255122c4b2e")
```
...
opgg_weekwr("Shakadevirgo07", "EUW", "RGAPI-aabdfe0a-76e1-494d-a2ff-3255122c4b2e")
```
